package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_986:Dictionary;
      ...
      private var var_985:int = 3;
      
      private var var_443:AssetLoaderStruct;
      
      private var var_984:Timer;
      ...
      private const const_2354:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_443 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_984 = new Timer(this.const_2354,1);
         this.var_984.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_443)
         {
            this.var_443.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_443.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_443)
         {
            this.var_443.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_443.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_985 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1224);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_985);
            if(this.var_1224.indexOf("?") > 0)
            {
               _loc2_ = this.var_1224 + "&retry=" + this.var_985;
            }
            else
            {
               _loc2_ = this.var_1224 + "?retry=" + this.var_985;
            ...
            this.var_443 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1825 = true;
         for each(_loc2_ in this.var_1225)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2464,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1223[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1825)
         {
            this.var_1225.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_598[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_598[_loc3_] = _loc6_;
            }
            this.var_986[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_986)
         {
            _loc7_ = true;
            _loc4_ = this.var_986[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_598[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_986[_loc3_];
         }
         while(_loc6_ < this.var_783.length)
         {
            _loc5_ = this.var_783[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_783.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_984.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_784.length > 0 && this.var_783.length < this.const_1761)
         {
            _loc1_ = this.var_784[0];
            _loc1_.startDownloading();
            this.var_783.push(this.var_784.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_784.indexOf(param1) == -1 && this.var_783.indexOf(param1) == -1)
         {
