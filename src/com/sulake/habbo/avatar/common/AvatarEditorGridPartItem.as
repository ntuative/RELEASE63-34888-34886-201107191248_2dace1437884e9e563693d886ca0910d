package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1863:BitmapData;
      
      private const const_2359:int = 13421772;
      
      private const const_2360:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_793 = param3;
         this._window = param1;
         this.var_1509 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1861 = param5;
         if(param3 == null)
         {
            this.var_748 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1862 = Math.max(this.var_1862,_loc6_.colorLayerIndex);
            }
         }
         this.var_447 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_748 != null && !this.var_1861)
            {
               _loc3_ = this.var_748;
            }
            else
            ...
            if(this.var_793)
            {
               switch(this.var_793.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_961;
                     break;
                  case HabboClubLevelEnum.const_43:
                     _loc2_.style = HabboIconType.const_736;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1509.color = this.const_2359;
         }
         else
         {
            this.var_1509.color = this.const_2360;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_447.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_447.isFigureReady(_loc1_))
         {
            this.var_447.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_387[_loc6_] + "_" + FigureData.const_172;
               _loc4_ = this.var_447.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_651 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1863)
               {
                  ...
                  this.var_1863 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_651.width,this.var_651.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_651.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_651.y;
               if(this.var_1861 && _loc2_.colorLayerIndex > 0)
               {
