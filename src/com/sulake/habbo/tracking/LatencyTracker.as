package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      {
         if(this.var_131 == null || this._communication == null || this._connection == null)
         {
            return;
         }
         this.var_1896 = int(this.var_131.getKey("latencytest.interval"));
         this.var_1894 = int(this.var_131.getKey("latencytest.report.index"));
         this.var_2587 = int(this.var_131.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1895 > this.var_1896)
         {
            this.testLatency();
         }
      }
      ...
      {
         this.var_1895 = getTimer();
         this.var_656.add(this.var_1528,this.var_1895);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1528));
         ++this.var_1528;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_656 == null || this.var_250 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_656.getValue(_loc2_.requestId);
         this.var_656.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_250.push(_loc4_);
         if(this.var_250.length == this.var_1894 && this.var_1894 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1893) > this.var_2587 || this.var_1893 == 0)
            {
               this.var_1893 = _loc9_;
