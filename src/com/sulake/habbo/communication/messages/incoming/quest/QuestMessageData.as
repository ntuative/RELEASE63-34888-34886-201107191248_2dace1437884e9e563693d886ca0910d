package com.sulake.habbo.communication.messages.incoming.quest
{
   import com.sulake.core.communication.messages.IMessageDataWrapper;
   
   public class QuestMessageData
   {
       
      
      private var var_2052:String;
      
      private var var_2055:int;
      
      private var var_2057:int;
      
      private var var_2132:int;
      
      private var _id:int;
      
      private var var_2053:Boolean;
      
      private var _type:String;
      
      private var var_2794:String;
      
      private var var_2795:int;
      
      private var var_2056:String;
      
      private var var_2797:int;
      
      private var var_2796:int;
      
      private var var_1608:int;
      
      private var var_2054:Date;
      
      public function QuestMessageData(param1:IMessageDataWrapper)
      {
         this.var_2054 = new Date();
         super();
         this.var_2052 = param1.readString();
         this.var_2055 = param1.readInteger();
         this.var_2057 = param1.readInteger();
         this.var_2132 = param1.readInteger();
         this._id = param1.readInteger();
         this.var_2053 = param1.readBoolean();
         this._type = param1.readString();
         this.var_2794 = param1.readString();
         this.var_2795 = param1.readInteger();
         this.var_2056 = param1.readString();
         this.var_2797 = param1.readInteger();
         this.var_2796 = param1.readInteger();
         this.var_1608 = param1.readInteger();
      }
      
      public function get campaignCode() : String
      {
         return this.var_2052;
      }
      
      public function get localizationCode() : String
      {
         return this.var_2056;
      }
      
      public function get completedQuestsInCampaign() : int
      {
         return this.var_2055;
      }
      
      public function get questCountInCampaign() : int
      {
         return this.var_2057;
      }
      
      public function get activityPointType() : int
      {
         return this.var_2132;
      }
      
      public function get accepted() : Boolean
      {
         return this.var_2053;
      }
      
      public function get id() : int
      {
         return this._id;
      }
      
      public function get type() : String
      {
         return this._type;
      }
      
      public function get imageVersion() : String
      {
         return this.var_2794;
      }
      
      public function get rewardCurrencyAmount() : int
      {
         return this.var_2795;
      }
      
      public function get completedSteps() : int
      {
         return this.var_2797;
      }
      
      public function get totalSteps() : int
      {
         return this.var_2796;
      }
      
      public function get waitPeriodSeconds() : int
      {
         if(this.var_1608 < 1)
         {
            return 0;
         }
         var _loc1_:Date = new Date();
         var _loc2_:int = _loc1_.getTime() - this.var_2054.getTime();
         return int(Math.max(0,this.var_1608 - Math.floor(_loc2_ / 1000)));
      }
      
      public function getCampaignLocalizationKey() : String
      {
         return "quests." + this.var_2052;
      }
      
      public function getQuestLocalizationKey() : String
      {
         return this.getCampaignLocalizationKey() + "." + this.var_2056;
      }
      
      public function get completedCampaign() : Boolean
      {
         return this._id < 1;
      }
      
      public function set id(param1:int) : void
      {
         this._id = param1;
      }
      
      public function set accepted(param1:Boolean) : void
      {
         this.var_2053 = param1;
      }
      
      public function get lastQuestInCampaign() : Boolean
      {
         return this.var_2055 >= this.var_2057;
      }
      
      public function get receiveTime() : Date
      {
         return this.var_2054;
      }
      
      public function set waitPeriodSeconds(param1:int) : void
      {
         this.var_1608 = param1;
      }
   }
}
