package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1637:int = 320;
      
      private static const const_1636:int = 48;
      
      private static const const_1635:int = 2;
       
      
      private var _view:IWindowContainer;
      
      private var var_64:IScrollableListWindow;
      ...
      {
         return var_933 as EffectsWidgetHandler;
      }
      ...
      public function open() : void
      {
         ...
         if(!this._view)
         {
            ...
            this._view.x = _loc2_.right + const_1635;
            this._view.y = _loc2_.bottom - this._view.height;
            this.var_64 = this._view.findChildByName("list") as IScrollableListWindow;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_404.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_404.add(_loc3_.type,_loc1_);
               this.var_64.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_404.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_404.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_64.removeListItem(_loc1_.window);
               this.var_404.remove(this.var_404.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_64.scrollableRegion.height;
         this.var_64.height = Math.max(Math.min(_loc5_,const_1637),const_1636);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1326,true);
      }
   }
}
