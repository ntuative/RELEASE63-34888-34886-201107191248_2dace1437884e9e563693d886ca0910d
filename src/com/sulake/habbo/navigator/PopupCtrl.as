package com.sulake.habbo.navigator
{
   ...
   public class PopupCtrl
   {
      ...
      {
         this.var_460 = new Timer(500,1);
         this.var_397 = new Timer(100,1);
         super();
         this._navigator = param1;
         this.var_1155 = param4;
         this.var_2743 = param2;
         this._popupIndentLeft = param3;
         this.var_460.addEventListener(TimerEvent.TIMER,this.onDisplayTimer);
         this.var_397.addEventListener(TimerEvent.TIMER,this.onHideTimer);
      }
      ...
      public function dispose() : void
      {
         ...
         if(this.var_460)
         {
            this.var_460.removeEventListener(TimerEvent.TIMER,this.onDisplayTimer);
            this.var_460.reset();
            this.var_460 = null;
         }
         if(this.var_397)
         {
            this.var_397.removeEventListener(TimerEvent.TIMER,this.onHideTimer);
            this.var_397.reset();
            this.var_397 = null;
         }
      }
      ...
      public function showPopup(param1:IWindow) : void
      {
         if(this.var_59 == null)
         {
            this.var_59 = IWindowContainer(this._navigator.getXmlWindow(this.var_1155));
            this.var_59.visible = false;
            this.var_59.setParamFlag(HabboWindowParam.const_44,true);
            this.var_59.procedure = this.onPopup;
         }
         Util.hideChildren(this.var_59);
         this.refreshContent(this.var_59);
         this.var_59.height = Util.getLowestPoint(this.var_59) + 5;
         var _loc2_:Point = new Point();
         param1.getGlobalPosition(_loc2_);
         this.var_59.x = _loc2_.x + this.var_2743 + param1.width;
         this.var_59.y = _loc2_.y - this.var_59.height * 0.5 + param1.height * 0.5;
         var _loc3_:Point = new Point();
         this.var_59.getGlobalPosition(_loc3_);
         if(_loc3_.x + this.var_59.width > this.var_59.desktop.width)
         {
            this.var_59.x = -this.var_59.width + _loc2_.x + this._popupIndentLeft;
      ...
      private function onPopup(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(!Util.containsMouse(this.var_59))
            {
               this.closePopup();
            }
         }
