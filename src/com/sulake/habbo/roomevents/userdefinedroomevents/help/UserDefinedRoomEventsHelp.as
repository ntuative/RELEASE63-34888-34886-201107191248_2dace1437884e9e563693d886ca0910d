package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1201:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1804 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1202 = this.var_1804.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1202)
         {
            this.var_1202.procedure = this.onButtonClick;
         }
         this.var_1201 = this.var_1804.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1201)
         {
            this.var_1201.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_274 >= this.var_73.length - 1)
         {
            this.var_1201.disable();
         }
         else
         {
            this.var_1201.enable();
         }
         if(this.var_274 <= 0)
         {
            this.var_1202.disable();
         }
         else
         {
            this.var_1202.enable();
         }
      }
      ...
      {
         ++this.var_274;
         this.var_274 = Math.min(this.var_274,this.var_73.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_274;
         this.var_274 = Math.max(this.var_274,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_93.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_73.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_73 || this.var_73.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_73[this.var_274][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_73[this.var_274][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_73[this.var_274][1]).value;
