package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1529:String = "asset-libraries";
      
      private static const const_1531:String = "library";
      
      private static const const_1533:String = "service-libraries";
      
      private static const const_1526:String = "library";
      
      private static const const_1527:String = "component-libraries";
      
      private static const const_1528:String = "library";
      
      private static const const_1525:String = "error_data";
      
      private static const const_1530:String = "error_cat";
      
      private static const const_1532:String = "error_desc";
       
      
      private var var_374:LibraryLoaderQueue;
      
      private var var_782:IEventDispatcher;
      
      private var var_531:uint;
      
      private var var_375:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1361:
               debug("Core; using simple frame update handler");
               this.var_375 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_928:
               debug("Core; using complex frame update handler");
               this.var_375 = this.complexFrameUpdateHandler;
               break;
            case Core.const_923:
               debug("Core; using profiler frame update handler");
               this.var_375 = this.profilerFrameUpdateHandler;
               this.var_217 = new Profiler(this);
               attachComponent(this.var_217,[new IIDProfiler()]);
               var_431.addChild(new ProfilerViewer(this.var_217));
               break;
            case Core.const_589:
               debug("Core; using experimental frame update handler");
               this.var_375 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_375 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_374.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_531 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_278[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1525,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1530,String(param3));
         ErrorReportStorage.setParameter(const_1532,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_782 = loadingEventDelegate;
         if(this.var_374 == null)
         {
            this.var_374 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1529)[0];
            if(node != null)
            {
               list = node.child(const_1531);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_212,e);
         }
         try
         {
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_212,e);
         }
         try
         {
            node = config.child(const_1527)[0];
            if(node != null)
            {
               list = node.child(const_1528);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_212,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1346);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_782)
         {
            this.var_782.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_782)
         {
            ...
            this.var_782.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_782)
                  {
                     this.var_782.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_531 - this.var_374.length,this.var_531,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_217 ? 0 : this.var_1491 & 0;
         if(_loc3_ == Core.const_589)
         {
            this.var_278[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_278[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_217 ? 0 : this.var_1491 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_278[_loc5_] as Array;
            if(_loc4_ == Core.const_589)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_375(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            receivers = this.var_278[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
